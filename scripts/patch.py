...
Additional modes:
    python3 scripts/patch.py --validate-only
    python3 scripts/patch.py --check-conflicts
    python3 scripts/patch.py --bundle identity <version> <release>
"""
...
class Patcher:
    ...
    def camoufox_patches(self):
        ...
        if getattr(options, 'bundle', None):
            features = [options.bundle]

        patch_files = list_patches(root_dir=patch_root, features=features or options.features)

        if options.validate_only:
            # Also run conflict detection during validation
            manifests = load_patch_manifests(root_dir=patch_root)
            conflicts = detect_conflicts(manifests, root_dir=patch_root)
            if conflicts:
                print_conflict_report(conflicts)
        ...
        # Log per-bundle statistics if a specific bundle was selected
        if features:
...
def extract_args():
    ...
    if options.validate_only and len(args) == 0:
        return "0", "0"
    if getattr(options, 'check_conflicts', False) and len(args) == 0:
        return "0", "0"
    if len(args) != 2:
...
if __name__ == "__main__":
    ...
    if options.validate_only or getattr(options, 'check_conflicts', False):
        patcher = Patcher("x86_64-pc-linux-gnu", "linux")
