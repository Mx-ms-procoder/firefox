...
def launch_options(
    ...
    **launch_options: Dict[str, Any],
    ...
    if env is None:
        import os as _os
        env = cast(Dict[str, Union[str, float, bool]], dict(_os.environ))
    else:
        env = cast(Dict[str, Union[str, float, bool]], dict(env))
    ...
    target_os = determine_ua_os(
        config.get('navigator.userAgent', fingerprint.navigator.userAgent)
    )
    browser_family = user_agent_parser.ParseUserAgent(
        config.get('navigator.userAgent', fingerprint.navigator.userAgent)
    ).get('family', 'Firefox')
    if browser_family != 'Firefox' and not i_know_what_im_doing:
        raise NonFirefoxFingerprint(
            f'"{browser_family}" identities are not supported in Camoufox. '
            'The runtime is Firefox-native, so cross-engine UA spoofing would '
            're-introduce TLS and network mismatches. '
            'If this is intentional, pass `i_know_what_im_doing=True`.'
        )

    if custom_fonts_only:
        firefox_user_prefs['gfx.bundled-fonts.activate'] = 0
        if fonts or config.get('fonts'):
    ...
    allow_webgl = launch_options.pop('allow_webgl', True)
    webgl_enabled = not (block_webgl or allow_webgl is False)
    ...
    # These are separate from CAMOU_CONFIG because NSS initializes
    # before Gecko's XPCOM/MaskConfig layer is ready.
    if _tls_profile:
        env_vars.update(get_tls_env_vars(_tls_profile))
