...
def _safe_extract_path(extract_root: Path, member: ZipInfo) -> Path:
    target_path = (extract_root / member.filename).resolve()
    if target_path != extract_root and extract_root not in target_path.parents:
        raise OSError(f"Refusing to extract archive member outside target directory: {member.filename}")
    return target_path
    ...
    with ZipFile(zip_file) as zf:
        for member in tqdm(
            ...
            if _is_symlink(member):
                raise OSError(f"Refusing to extract symlink from archive: {member.filename}")

            target_path = _safe_extract_path(extract_root, member)
            if member.is_dir():
