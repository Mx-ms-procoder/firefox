...
@click.option(
    ...
    help='Directory used for persisted session snapshots',
)
@click.option('--pool-size', default=4, show_default=True, type=int, help='Warm worker pool size')
def cloud_broker(host: str, port: int, snapshot_dir: Optional[str], pool_size: int) -> None:
    """
    Launch the stateless session broker used by the cloud-native pool model.
    """
    from .cloud_native import serve_broker

    serve_broker(host=host, port=port, snapshot_dir=snapshot_dir, pool_size=pool_size)
